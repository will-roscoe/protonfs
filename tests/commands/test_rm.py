...
def test_rm_trashes_and_removes_from_index(tmp_path: Path, make_fake_drive) -> None:
...
def test_rm_force_deletes_when_exactly_one_trash_match(tmp_path: Path, make_fake_drive) -> None:
    ...
    init_config(tmp_path, "/my-files/test")
    ctx = load_context(tmp_path)
    fake = make_fake_drive()  # default listing: one entry per trashed basename
    ctx.drive = fake
...
def test_rm_force_duplicate_basename_leaves_trashed_and_warns(
...
def test_rm_force_zero_trash_matches_informs_and_keeps_trashed(
    ...
    init_config(tmp_path, "/my-files/test")
    ctx = load_context(tmp_path)
    fake = make_fake_drive(trash_listing=[])  # nothing matches the basename
    ctx.drive = fake
...
def test_rm_directory_without_recursive_raises(tmp_path: Path, make_fake_drive) -> None:
    (tmp_path / "run1").mkdir()
...
def test_rm_directory_with_recursive_removes_all_index_entries_under_it(
...
def test_rm_unconfirmed_triggers_confirmation_prompt(
...
def test_rm_confirmed_does_not_trigger_confirmation_prompt(
    tmp_path: Path, monkeypatch, make_fake_drive
) -> None:
    init_config(tmp_path, "/my-files/test")
    ctx = load_context(tmp_path)
...
def test_rm_recursive_does_not_remove_sibling_directories(
    tmp_path: Path, make_fake_drive
) -> None:
    (tmp_path / "run1").mkdir()
    (tmp_path / "run1" / "dump_0001").write_bytes(b"data")
