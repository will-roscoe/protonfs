...
def test_restore_narrates_item(tmp_path: Path, make_fake_drive, recording_reporter_cls) -> None:
    init_config(tmp_path, "/my-files/test")
    ctx = load_context(tmp_path)
    ctx.index.set(
        "dump_0001",
    ...
    restore(ctx, "dump_0001", reporter=rep)

    assert rep.calls == [("item", "dump_0001")]
